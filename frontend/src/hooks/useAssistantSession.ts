...
import { createReconnectScheduler } from "@/utils/reconnect-scheduler";
import type {
  AssistantSnapshot,
  PendingQuestion,
  SessionMeta,
...
export function useAssistantSession(projectName: string | null) {
  ...
  const reconnectSchedulerRef = useRef(createReconnectScheduler());
  ...
  const closeStream = useCallback(() => {
    reconnectSchedulerRef.current.cancel();
  ...
  const connectStream = useCallback(
    (sessionId: string) => {
      if (!pageVisible || isAuthSessionExpired()) return;
      ...
      source.onerror = () => {
        if (!isActiveStream()) return;
        if (isAuthSessionExpired() || !pageVisible) {
          closeStream();
          return;
        }
        ...
        if (statusRef.current === "running" || store.getState().sending) {
          reconnectSchedulerRef.current.schedule(() => {
            if (!isActiveStream() || isAuthSessionExpired() || !pageVisible) return;
    ...
    [applySnapshot, clearPendingQuestion, pageVisible, projectName, closeStream, store, syncPendingQuestion],
  );
  ...
  useEffect(() => {
    ...
    if (
      ...
      (!streamRef.current || streamRef.current.readyState === EventSource.CLOSED)
    ) {
      connectStream(sessionId);
    }
  }, [pageVisible, projectName, connectStream, closeStream, store]);
