...
describe("LoginPage returnTo consumption", () => {
  ...
  it("falls back to /app/home when ?from is an unsafe open-redirect target", async () => {
    const { container, history } = renderLoginAt("/login?from=https%3A%2F%2Fevil.com%2Fapp%2Fx");
    submitLogin(container);
    await waitFor(() => {
      expect(history.at(-1)).toBe("/app/home");
    });
  });

  it("falls back to /app/home when no ?from is present", async () => {
