...
import { ChevronLeft, ListChecks, Settings, Bell, Download, Loader2, Package } from "lucide-react";
...
import { W3_ACCENT_BUTTON_STYLE, W3_HEADER_BAR_STYLE } from "@/components/workspace";
...
export function GlobalHeader({ onNavigateBack }: GlobalHeaderProps) {
  ...
  const [mergedVideoExporting, setMergedVideoExporting] = useState(false);
  ...
  const handleMergedVideoExport = async (episode: number) => {
    if (!currentProjectName || mergedVideoExporting) return;

    setMergedVideoExporting(true);
    try {
      const { download_token } = await API.requestExportToken(currentProjectName, "current");
      const url = API.getMergedVideoDownloadUrl(currentProjectName, episode, download_token);
      triggerBrowserDownload(url);
      setExportDialogOpen(false);
      useAppStore.getState().pushToast(t("dashboard:merged_video_export_started"), "success");
    } catch (err) {
      useAppStore
        .getState()
        .pushNotification(t("dashboard:export_failed", { message: errMsg(err) }), "error");
    } finally {
      setMergedVideoExporting(false);
    }
  };
      ...
      >
        ...
        <div className="flex items-center gap-1">
          ...
          <div className="relative" ref={taskHudAnchorRef}>
            ...
            >
              <ListChecks className={`h-4 w-4 ${runningCount > 0 ? "animate-shot-pulse" : ""}`} />
              {runningCount > 0 && (
                <span
                  className="absolute -right-1 -top-1 flex h-4 min-w-4 items-center justify-center rounded-full px-1 text-[10px] font-bold"
          ...
          <div className="relative" ref={exportAnchorRef}>
            <button
              ...
              className="inline-flex items-center gap-1.5 rounded-[7px] px-3 py-1.5 text-xs font-semibold transition-transform focus-ring disabled:cursor-not-allowed disabled:opacity-50 motion-safe:hover:-translate-y-px"
            ...
            <ExportScopeDialog
              ...
              onSelect={(scope) => {
                if (scope !== "jianying-draft" && scope !== "merged-video") void handleExportProject(scope);
              }}
              ...
              onMergedVideoExport={voidPromise(handleMergedVideoExport)}
              mergedVideoExporting={mergedVideoExporting}
            />
          </div>
          ...
          <button
            ...
            onClick={() => {
              ...
              setLocation(absoluteAppPath("/app/assets"));
            }}
          ...
          <button
            ...
            onClick={() =>
              setLocation(
                currentProjectName
                  ? absoluteAppPath(`/app/projects/${encodeURIComponent(currentProjectName)}/settings`)
                  : absoluteAppPath("/app/settings"),
              )
            }
            className="relative grid h-[30px] w-[30px] place-items-center rounded-md transition-colors focus-ring"
            style={{ color: "var(--color-text-3)" }}
