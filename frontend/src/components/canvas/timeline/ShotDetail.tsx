...
interface ShotDetailProps {
  ...
  onUploadStoryboard?: (segmentId: string) => void | Promise<void>;
...
export function ShotDetail({
  ...
  const isAuthorized = segment.generated_assets?.authorized === true;
  ...
  const handleConfirmUploadStoryboard = async () => {
    if (!onUploadStoryboard) return;
    setUploadConfirmOpen(false);
    try {
      await onUploadStoryboard(segmentId);
    } catch {
  ...
  return (
    ...
    >
      ...
      >
        ...
        {onUploadStoryboard && (
          <button
            ...
            disabled={storyboardUploadBusy || dirty || saving || isAuthorized}
            title={
              isAuthorized
      ...
      <ConfirmDialog
        ...
        onConfirm={() => void handleConfirmUploadStoryboard()}
