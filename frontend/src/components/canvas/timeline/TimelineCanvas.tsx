...
interface TimelineCanvasProps {
  ...
  onGenerateStoryboardsBatch?: (scriptFile?: string) => void;
  onGenerateVideosBatch?: (scriptFile?: string) => void;
  onUploadStoryboard?: (segmentId: string, scriptFile?: string) => void | Promise<void>;
  onUploadStoryboardsBatch?: (scriptFile?: string) => void | Promise<void>;
...
export function TimelineCanvas({
  ...
  onGenerateStoryboardsBatch,
  onGenerateVideosBatch,
  onUploadStoryboard,
  onUploadStoryboardsBatch,
  onRestoreStoryboard,
  onRestoreVideo,
  onSaveTitle,
  canEditTitle,
  ...
  const isGenerating = useCallback(
    (taskType: "storyboard" | "video" | "tts" | "storyboard_upload", segmentId: string): boolean =>
      tasks.some(
        (t) =>
  ...
  const uploadingStoryboard = useCallback(
    (segId: string) => isGenerating("storyboard_upload", segId),
    [isGenerating],
  );
  ...
  const storyboardUploadBatchBusy = useMemo(
    () =>
      storyboardUploadBlocking ||
      tasks.some(
        (t) =>
          t.task_type === "storyboard_upload" &&
          t.project_name === projectName &&
          currentSegmentIds.has(t.resource_id) &&
          (t.status === "queued" || t.status === "running"),
      ),
    [tasks, projectName, currentSegmentIds, storyboardUploadBlocking],
  );

  const handleConfirmBatchUpload = useCallback(async () => {
    if (!onUploadStoryboardsBatch) return;
    setBatchUploadConfirmOpen(false);
    try {
      await onUploadStoryboardsBatch(scriptFile);
    } catch {
      // toast 由 StudioCanvasRouter 统一弹出
    }
  }, [onUploadStoryboardsBatch, scriptFile]);

  const handleUploadStoryboardForSegment = useCallback(
    (segmentId: string) => onUploadStoryboard?.(segmentId, scriptFile),
    [onUploadStoryboard, scriptFile],
  );
  ...
  return (
    <div className="flex h-full flex-col overflow-hidden">
        ...
        {activeTab === "timeline" && hasScript && (
          <div className="mr-1 inline-flex items-center gap-1.5">
            <button
              ...
              disabled={!onGenerateStoryboardsBatch}
              onClick={() => onGenerateStoryboardsBatch?.(scriptFile)}
            ...
            <button
              type="button"
              className="sv-navbtn inline-flex items-center gap-1.5"
              disabled={!onUploadStoryboardsBatch || storyboardUploadBatchBusy}
              onClick={() => setBatchUploadConfirmOpen(true)}
              title={t("batch_upload_storyboards")}
            >
              {storyboardUploadBatchBusy ? (
                <Loader2 className="h-3 w-3 animate-spin" />
              ) : (
                <Upload className="h-3 w-3" />
              )}
              <span>{t("batch_upload_storyboards")}</span>
            </button>
            <button
              type="button"
              className="sv-navbtn inline-flex items-center gap-1.5"
              disabled={!onGenerateVideosBatch}
              onClick={() => onGenerateVideosBatch?.(scriptFile)}
          ...
          <div className="flex h-full flex-col">
            ...
            <div className="min-h-0 flex-1 overflow-hidden">
              <ShotSplitView
                ...
                onUploadStoryboard={handleUploadStoryboardForSegment}
      ...
      <ConfirmDialog
        ...
        onConfirm={() => void handleConfirmBatchUpload()}
