...
import { Pencil, RefreshCw, Sparkles, Users, Landmark, Package, Wand2 } from "lucide-react";
...
export function OverviewCanvas({ projectName, projectData }: OverviewCanvasProps) {
  ...
  const [autoGeneratingAssets, setAutoGeneratingAssets] = useState<Record<string, boolean>>({
    character: false,
    scene: false,
    prop: false,
  });
  ...
  const handleDirectGenerate = useCallback(
    async () => {
      ...
      try {
        const result = await API.directGenerateEpisode(projectName, {
          episode: 1,
          title: null,
        });
        if (result.success) {
          useAppStore
            .getState()
            .pushToast(tRef.current("direct_generate_success", { title: result.title || "" }), "success");
          await refreshProject();
        } else {
          useAppStore.getState().pushToast(result.message || "生成失败", "error");
        }
      } catch (e) {
        useAppStore.getState().pushToast(errMsg(e), "error");
      } finally {
  ...
  const handleAutoGenerateAssets = useCallback(
    async (assetType: "character" | "scene" | "prop") => {
      setAutoGeneratingAssets((prev) => ({ ...prev, [assetType]: true }));
      try {
        const result = await API.autoGenerateAssets(projectName, {
        ...
        if (result.success) {
          ...
          if (taskId) {
            ...
            useAppStore
              .getState()
              .pushToast(
                tRef.current("auto_generate_assets_success", {
                  type: tRef.current(assetType === "character" ? "characters" : assetType === "scene" ? "scenes" : "props"),
                  count,
                }),
                "success"
              );
        ...
        setAutoGeneratingAssets((prev) => ({ ...prev, [assetType]: false }));
      }
    },
    [projectName, refreshProject],
  );
                  ...
                  return (
                    ...
                    >
                      ...
                      <button
                        type="button"
                        onClick={() => {
                          const assetType = key === "characters" ? "character" : key === "scenes" ? "scene" : "prop";
                          void handleAutoGenerateAssets(assetType);
                        }}
                        disabled={autoGeneratingAssets[key === "characters" ? "character" : key === "scenes" ? "scene" : "prop"]}
                        className="mt-2 flex w-full items-center justify-center gap-1 rounded-lg px-2 py-1 text-[11px] font-medium transition-colors focus-ring disabled:opacity-50"
                        style={{
                          background: "var(--color-accent-dim)",
                          color: "var(--color-accent-2)",
                          border: "1px solid var(--color-accent-soft)",
                      ...
                      >
                        ...
                        {autoGeneratingAssets[key === "characters" ? "character" : key === "scenes" ? "scene" : "prop"]
                          ? t("generating_assets")
                          : t("generate_assets", { type: labels[key] })}
                      </button>
                    </div>
                  );
                })}
              ...
              {(projectData.episodes?.length ?? 0) === 0 ? (
                <div className="flex flex-col items-center gap-3 py-2">
                  ...
                  <button
                    ...
                    className="flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-[12px] font-medium transition-colors focus-ring disabled:opacity-50"
                    style={{
