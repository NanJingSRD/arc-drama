...
import { REFERENCE_SHOT_DURATION_OPTIONS, type EpisodeScript } from "@/types/script";
import { upsertOptimisticAssetTask } from "@/utils/optimistic-asset-task";
...
function collectScriptSegmentIds(script: EpisodeScript): string[] {
  if (script.content_mode === "narration") {
    return script.segments.map((s) => s.segment_id);
  }
  if (script.content_mode === "ad") {
    return script.shots.map((s) => s.shot_id);
  }
  return script.scenes.map((s) => s.scene_id);
}
...
export function StudioCanvasRouter() {
  ...
  const handleGenerateStoryboardsBatch = useCallback(async (scriptFile?: string) => {
    if (!currentProjectName || !currentScripts) return;
    const resolvedFile = scriptFile ?? Object.keys(currentScripts)[0];
    if (!resolvedFile) return;
    const script = currentScripts[resolvedFile];
    ...
    const contentMode = script.content_mode;
    ...
    if (contentMode === "narration") {
      for (const seg of script.segments) {
        if (!seg.generated_assets?.storyboard_image) {
          idsToGenerate.push(seg.segment_id);
          prompts.push(typeof seg.image_prompt === "string" ? seg.image_prompt : (seg.image_prompt as unknown as Record<string, unknown>));
        }
      }
    } else if (contentMode === "ad") {
      for (const seg of script.shots) {
        if (!seg.generated_assets?.storyboard_image) {
          idsToGenerate.push(seg.shot_id);
          prompts.push(typeof seg.image_prompt === "string" ? seg.image_prompt : (seg.image_prompt as unknown as Record<string, unknown>));
        }
      }
    } else {
      for (const seg of script.scenes) {
        if (!seg.generated_assets?.storyboard_image) {
          idsToGenerate.push(seg.scene_id);
          prompts.push(typeof seg.image_prompt === "string" ? seg.image_prompt : (seg.image_prompt as unknown as Record<string, unknown>));
        }
      }
    }
    ...
    for (let i = 0; i < idsToGenerate.length; i++) {
      try {
        await API.generateStoryboard(currentProjectName, idsToGenerate[i], prompts[i], resolvedFile);
        successCount++;
      } catch {
        failCount++;
      }
    ...
    if (failCount === 0) {
      useAppStore.getState().pushToast(tRef.current("storyboard_batch_submitted_toast", { count: successCount }), "success");
    } else {
      useAppStore.getState().pushToast(
        `${successCount} 成功，${failCount} 失败`,
        failCount === idsToGenerate.length ? "error" : "warning",
      );
    }
  }, [currentProjectName, currentScripts]);

  const handleUploadStoryboard = useCallback(async (segmentId: string) => {
    if (!currentProjectName) return;
    useAppStore.getState().setBlockingOverlay(
      tRef.current("storyboard_upload_loading", { id: segmentId }),
    );
    try {
      await API.uploadStoryboard(currentProjectName, segmentId);
      await refreshProject();
      useAppStore.getState().pushToast(
        tRef.current("storyboard_upload_submitted_toast", { id: segmentId }),
        "success",
      );
    } catch (err) {
      useAppStore.getState().pushToast(
        tRef.current("upload_storyboard_failed", { message: errMsg(err) }),
        "error",
      );
      throw err;
    } finally {
  ...
  const handleUploadStoryboardsBatch = useCallback(async (scriptFile?: string) => {
    if (!currentProjectName || !currentScripts) return;
    const resolvedFile = scriptFile ?? Object.keys(currentScripts)[0];
    if (!resolvedFile) return;
    const script = currentScripts[resolvedFile];
    ...
    const segmentIds = collectScriptSegmentIds(script);
    if (segmentIds.length === 0) return;

    useAppStore.getState().setBlockingOverlay(
      tRef.current("storyboard_upload_batch_loading", { count: segmentIds.length }),
    );
    try {
      const res = await API.uploadStoryboardsBatch(currentProjectName, segmentIds);
      await refreshProject();
      useAppStore.getState().pushToast(
        tRef.current("storyboard_upload_batch_submitted_toast", {
          count: res.task_ids?.length ?? segmentIds.length,
        }),
        "success",
      );
    } catch (err) {
  ...
  const handleGenerateVideosBatch = useCallback(async (scriptFile?: string) => {
    if (!currentProjectName || !currentScripts) return;
    const resolvedFile = scriptFile ?? Object.keys(currentScripts)[0];
    if (!resolvedFile) return;
    const script = currentScripts[resolvedFile];
    ...
    if (contentMode === "narration") {
      for (const seg of script.segments) {
        if (seg.generated_assets?.storyboard_image && !seg.generated_assets?.video_clip) {
          idsToGenerate.push(seg.segment_id);
          prompts.push(typeof seg.video_prompt === "string" ? seg.video_prompt : (seg.video_prompt as unknown as Record<string, unknown>));
          durations.push(seg.duration_seconds ?? 4);
        }
      }
    } else if (contentMode === "ad") {
      for (const seg of script.shots) {
    ...
    for (let i = 0; i < idsToGenerate.length; i++) {
      try {
        await API.generateVideo(currentProjectName, idsToGenerate[i], prompts[i], resolvedFile, durations[i]);
        successCount++;
      } catch {
        failCount++;
      }
    ...
    if (failCount === 0) {
      useAppStore.getState().pushToast(tRef.current("video_batch_submitted_toast", { count: successCount }), "success");
    } else {
      useAppStore.getState().pushToast(
        `${successCount} 成功，${failCount} 失败`,
        failCount === idsToGenerate.length ? "error" : "warning",
  ...
  const handleGenerateCharacter = useCallback(async (name: string, context?: { description?: string }) => {
    if (!currentProjectName) return;
    try {
      const res = await API.generateCharacter(
        currentProjectName,
        name,
        context?.description ?? currentProjectData?.characters?.[name]?.description ?? "",
      );
  ...
  const handleGenerateScene = useCallback(async (name: string, context?: { description?: string }) => {
    if (!currentProjectName) return;
    try {
      const res = await API.generateProjectScene(currentProjectName, name, context?.description ?? currentProjectData?.scenes?.[name]?.description ?? "");
      upsertOptimisticAssetTask({
        taskId: res.task_id,
        projectName: currentProjectName,
        taskType: "scene",
  ...
  const handleGenerateProp = useCallback(async (name: string, context?: { description?: string }) => {
    if (!currentProjectName) return;
    try {
      const res = await API.generateProjectProp(currentProjectName, name, context?.description ?? currentProjectData?.props?.[name]?.description ?? "");
      upsertOptimisticAssetTask({
        taskId: res.task_id,
        projectName: currentProjectName,
        taskType: "prop",
              ...
              <div className="min-h-0 flex-1">
                {mode === "reference_video" && !isAd ? (
                  ...
                  <GridImageToVideoCanvas
                    ...
                    onUploadStoryboard={voidPromise(handleUploadStoryboard)}
                  ...
                  <TimelineCanvas
                    ...
                    onGenerateNarration={voidPromise(handleGenerateNarration)}
                    onGenerateEpisodeNarration={voidPromise(handleGenerateEpisodeNarration)}
                    onGenerateStoryboardsBatch={handleGenerateStoryboardsBatch}
                    onGenerateVideosBatch={handleGenerateVideosBatch}
                    onUploadStoryboard={voidPromise(handleUploadStoryboard)}
                    onUploadStoryboardsBatch={voidPromise(handleUploadStoryboardsBatch)}
                    onRestoreStoryboard={handleRestoreAsset}
                    onRestoreVideo={handleRestoreAsset}
                  />
                )}
