...
describe("WelcomeCanvas delete source file", () => {
  beforeEach(() => {
    useAppStore.setState(useAppStore.getInitialState(), true);
    vi.restoreAllMocks();
  });

  it("calls onDeleteSourceFile and removes the row", async () => {
    const onDeleteSourceFile = vi.fn().mockResolvedValue(undefined);
    const onUpload = vi.fn().mockResolvedValue(undefined);

    renderWelcome({
      onUpload,
      onDeleteSourceFile,
      skipSourceFilesFetch: true,
      welcomeVariant: "workspace-v2",
    });

    const dropZone = (await screen.findByText(/拖拽文件到此处/)).closest("button");
    expect(dropZone).not.toBeNull();
    const file = new File(["x"], "gaolaozhuang.txt", { type: "text/plain" });
    fireEvent.drop(dropZone as HTMLElement, { dataTransfer: { files: [file] } });

    await waitFor(() => expect(onUpload).toHaveBeenCalledWith(file));
    expect(await screen.findByText("gaolaozhuang.txt")).toBeInTheDocument();

    fireEvent.click(
      screen.getByRole("button", { name: /删除源文件 gaolaozhuang\.txt/ }),
    );

    expect(
      await screen.findByText(/确定要删除文件 "gaolaozhuang\.txt" 吗/),
    ).toBeInTheDocument();

    fireEvent.click(screen.getByRole("button", { name: "删除" }));

    await waitFor(() => {
      expect(onDeleteSourceFile).toHaveBeenCalledWith("gaolaozhuang.txt");
    });
    await waitFor(() => {
      expect(screen.queryByText("gaolaozhuang.txt")).not.toBeInTheDocument();
    });
  });
});
