...
const CredentialRow = memo(function CredentialRow({
  ...
  const handleActivate = useCallback(async () => {
    try {
      await WorkspaceV2SettingsAPI.activateCredential(providerId, cred.id);
      onChanged();
    } catch {
      // 网络错误静默处理
    }
  ...
  const handleTest = useCallback(async () => {
    ...
    try {
      const result = await WorkspaceV2SettingsAPI.testProviderConnection(providerId, cred.id);
      if (compact) {
  ...
  const handleDelete = useCallback(async () => {
    ...
    setDeleting(true);
    try {
      await WorkspaceV2SettingsAPI.deleteCredential(providerId, cred.id);
      onChanged();
    } finally {
      setDeleting(false);
  ...
  const handleSaveEdit = useCallback(async () => {
    ...
    setSaving(true);
    try {
      await WorkspaceV2SettingsAPI.updateCredential(providerId, cred.id, data);
      setEditing(false);
      onChanged();
    } finally {
      setSaving(false);
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (isVertex) {
        ...
        await WorkspaceV2SettingsAPI.uploadVertexCredential(name, file);
      } else {
        ...
        await WorkspaceV2SettingsAPI.createCredential(providerId, payload);
      }
      onCreated();
    } catch (e) {
      showError(errMsg(e));
...
export function WorkspaceV2CredentialList({
  providerId,
  supportsBaseUrl,
  secretFields,
  onChanged,
  ...
  const refresh = useCallback(async () => {
    try {
      const { credentials: creds } = await WorkspaceV2SettingsAPI.listCredentials(providerId);
