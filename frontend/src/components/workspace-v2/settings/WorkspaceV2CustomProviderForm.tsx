...
import { useWorkspaceV2EndpointCatalogStore } from "@/stores/workspace-v2-endpoint-catalog-store";
...
import { priceLabel, urlPreviewFor, toggleDefaultReducer, type DiscoveryFormat } from "@/components/pages/settings/customProviderHelpers";
import { WorkspaceV2EndpointSelect } from "./WorkspaceV2EndpointSelect";
...
const COMPACT_PRICING_CONTROL_CLS =
  "h-7 min-w-0 shrink-0 rounded-[6px] border border-hairline bg-bg-grad-a/55 px-2 text-[12px] text-text transition-colors hover:border-hairline-strong focus:border-accent/55 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent";
...
const DISCOVERY_FORMAT_OPTIONS: { value: DiscoveryFormat; labelKey: string }[] = [
  ...
  { value: "custom", labelKey: "discovery_format_custom" },
];
...
interface CustomProviderFormProps {
  ...
  onSaved: () => void;
  onCancel: () => void;
  /** 嵌入工作空间 2.0 弹框：去掉页面级 padding，与预置供应商对齐 */
  embedded?: boolean;
}

export function WorkspaceV2CustomProviderForm({
  existing,
  onSaved,
  onCancel,
  embedded = false,
  ...
  const endpointToMediaType = useWorkspaceV2EndpointCatalogStore((s) => s.endpointToMediaType);
  const endpointToImageCapabilities = useWorkspaceV2EndpointCatalogStore((s) => s.endpointToImageCapabilities);
  const fetchEndpointCatalog = useWorkspaceV2EndpointCatalogStore((s) => s.fetch);
  useEffect(() => {
  ...
  const handleDiscover = useCallback(async () => {
    ...
    try {
      const res = useStoredCredential
        ? await WorkspaceV2SettingsAPI.discoverModelsForProvider(existing.id)
        : await WorkspaceV2SettingsAPI.discoverModels({ discovery_format: discoveryFormat, base_url: baseUrl, api_key: apiKey });
  ...
  const handleTest = useCallback(async () => {
    if (!embedded) setTestResult(null);
    if (!baseUrl) {
      showError(t("fill_base_url_first"));
      return;
    }
    ...
    setTesting(true);
    try {
      const res = useStoredCredential && existing
        ? await WorkspaceV2SettingsAPI.testCustomConnectionById(existing.id)
        : await WorkspaceV2SettingsAPI.testCustomConnection({ discovery_format: discoveryFormat, base_url: baseUrl, api_key: apiKey });
      if (embedded) {
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      if (isEdit && existing) {
        ...
        await WorkspaceV2SettingsAPI.fullUpdateCustomProvider(existing.id, {
        ...
        await WorkspaceV2SettingsAPI.createCustomProvider({
          display_name: displayName,
    ...
    <>
      <div className={embedded ? "w-full" : "max-w-2xl"}>
        <div className="mb-6">
          ...
          <h3
            className={embedded ? "text-[15px] font-medium leading-[1.55] text-text" : "font-editorial mt-1"}
            style={
              embedded
                ? undefined
                : {
                  ...
                  >
                    ...
                    >
                      ...
                      <input
                        ...
                        className={`${COMPACT_INPUT_CLS} min-w-0 ${embedded ? "" : "flex-1"}`}
                      />

                      {!embedded ? (
                        <WorkspaceV2EndpointSelect
                          value={m.endpoint}
                      ...
                      <button
                        ...
                        className="shrink-0 rounded-[6px] px-2 py-1 font-mono text-[10px] font-bold uppercase tracking-[0.14em] transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent"
                        style={
                          m.is_default
                            ? {
                      ...
                      <button
                        ...
                        className="shrink-0 rounded p-1 text-text-4 transition-colors hover:text-warm-bright focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent"
                    ...
                    {/* Pricing row */}
                    <div className="mt-2 flex flex-wrap items-center gap-2 pl-6 text-[12px] text-text-4">
                      <select
                        value={m.currency}
                        onChange={(e) => updateModel(m.key, { currency: e.target.value })}
                        aria-label={t("currency_label")}
                        className={`${COMPACT_PRICING_CONTROL_CLS} w-9`}
                      >
                      ...
                      <input
                        ...
                        className={`${COMPACT_PRICING_CONTROL_CLS} w-16`}
                      />
                      <span className="flex h-7 items-center">{pl.input}</span>
                      {pl.output && (
                        <>
                          <span className="flex h-7 items-center text-text-4">|</span>
                          <input
                            type="text"
                            inputMode="decimal"
                            ...
                            className={`${COMPACT_PRICING_CONTROL_CLS} w-16`}
                          />
                          <span className="flex h-7 items-center">{pl.output}</span>
                        </>
                      )}
                    </div>
  ...
  if (embedded) {
    return (
      <div className="flex min-h-0 flex-1 flex-col">
        ...
        <div className="z-10 shrink-0 border-t border-hairline pt-3">{actionBar}</div>
      </div>
    );
  }
  ...
  return (
    <div>
      <div className="p-6 pb-24">{formBody}</div>
      <div
