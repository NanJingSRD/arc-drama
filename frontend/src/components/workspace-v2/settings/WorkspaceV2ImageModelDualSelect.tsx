...
const DUAL_HEADER_LABEL_COMPACT_CLS =
...
export function WorkspaceV2ImageModelDualSelect({
  valueT2I,
  valueI2I,
  options,
  providerNames,
  ...
  const endpointToCaps = useWorkspaceV2EndpointCatalogStore((s) => s.endpointToImageCapabilities);
  ...
  const dualHint = showCapabilityHint ? (
    <p className={`leading-[1.5] text-text-4 ${compact ? "text-[10px]" : "text-[11.5px]"}`}>
      {t("model_image_dual_hint")}
    </p>
  ) : null;

  const dualHeaderLabelCls = compact ? DUAL_HEADER_LABEL_COMPACT_CLS : DUAL_HEADER_LABEL_CLS;
  ...
  return (
    <div className="space-y-3">
      <div>
        <div className={dualHeaderLabelCls}>{labelT2I ?? t("model_image_t2i")}</div>
        <ProviderModelSelect
          value={valueT2I}
          options={t2iOptions}
          providerNames={providerNames}
      ...
      <div>
        <div className={dualHeaderLabelCls}>{labelI2I ?? t("model_image_i2i")}</div>
        <ProviderModelSelect
          value={valueI2I}
          options={i2iOptions}
          providerNames={providerNames}
