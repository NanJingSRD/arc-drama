...
import { WorkspaceV2CredentialList } from "./WorkspaceV2CredentialList";
...
function FieldEditor({ field, draft, setDraft, compact = false }: FieldEditorProps) {
  const { t } = useTranslation("dashboard");
    ...
    return (
      <div>
        ...
        <input
          ...
          className={compact ? inputCls : `${INPUT_CLS} max-w-[140px]`}
        />
      </div>
    );
  }
...
interface Props {
  ...
  /** 覆盖密钥区「添加」按钮文案（工作空间 2.0） */
  addCredentialLabel?: string;
}

export function WorkspaceV2ProviderDetail({
  providerId,
  onSaved,
  centerLoading = false,
  addCredentialLabel,
  ...
  const handleCredentialChanged = useCallback(async () => {
    const updated = await WorkspaceV2SettingsAPI.getProviderConfig(providerId);
    setDetail(updated);
    onSaved?.();
  }, [providerId, onSaved]);
  ...
  useEffect(() => {
    ...
    setSaveError(null);
    voidCall(
      WorkspaceV2SettingsAPI.getProviderConfig(providerId)
        .then((res) => {
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      await WorkspaceV2SettingsAPI.patchProviderConfig(providerId, patch);
      const updated = await WorkspaceV2SettingsAPI.getProviderConfig(providerId);
      setDetail(updated);
      setDraft({});
      onSaved?.();
  ...
  }, [draft, providerId, onSaved, centerLoading]);
  ...
  return (
    <div className={centerLoading ? "w-full space-y-6" : "max-w-2xl space-y-6"}>
      ...
      {/* Credentials */}
      <WorkspaceV2CredentialList
        providerId={providerId}
        supportsBaseUrl={detail.supports_base_url}
        secretFields={detail.secret_fields}
        addCredentialLabel={addCredentialLabel}
        compact={centerLoading}
        ...
        <div>
          ...
          {showAdvanced && (
            <div
              className={
                centerLoading
                  ? "mt-3 grid grid-cols-1 gap-x-5 gap-y-3 sm:grid-cols-2"
                  : "mt-3 space-y-4"
              }
            >
              {detail.fields.map((field) => (
                <FieldEditor
                  ...
                  field={field}
                  draft={draft}
                  setDraft={handleDraftEdit}
                  compact={centerLoading}
                />
              ))}
              {hasDraft && (
                <div className="pt-1">
                  {saveError && !centerLoading && (
                    <p
                      aria-live="polite"
