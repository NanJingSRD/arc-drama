...
interface EndpointSelectProps {
  ...
  /** Disable interaction */
  disabled?: boolean;
  /** 窄容器（工作空间 2.0 弹框）：隐藏路径提示、占满可用宽度 */
  compact?: boolean;
}

export function WorkspaceV2EndpointSelect({
  value,
  onChange,
  ariaLabel,
  disabled,
  ...
  const endpoints = useWorkspaceV2EndpointCatalogStore((s) => s.endpoints);
  const initialized = useWorkspaceV2EndpointCatalogStore((s) => s.initialized);
  const loading = useWorkspaceV2EndpointCatalogStore((s) => s.loading);
  const fetchCatalog = useWorkspaceV2EndpointCatalogStore((s) => s.fetch);
  const refreshCatalog = useWorkspaceV2EndpointCatalogStore((s) => s.refresh);
  ...
  return (
    <>
      <button
        ...
        className={[
          compact ? "flex w-full min-w-0 items-center gap-1.5" : "group inline-flex items-center gap-2",
          "rounded-[8px] border px-2.5 py-1.5 text-left text-sm transition-colors",
      ...
      >
        <span className={`min-w-0 truncate ${compact ? "text-[12px]" : ""}`}>{triggerLabel}</span>
        {triggerHint && !compact && (
          <span
            aria-hidden="true"
