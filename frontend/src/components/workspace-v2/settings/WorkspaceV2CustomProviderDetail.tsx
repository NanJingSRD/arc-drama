...
import { WorkspaceV2CustomProviderForm } from "./WorkspaceV2CustomProviderForm";
...
export function WorkspaceV2CustomProviderDetail({
  providerId,
  onDeleted,
  onSaved,
  centerLoading = false,
  ...
  const fetchProvider = useCallback(async () => {
    setLoading(true);
    try {
      const data = await WorkspaceV2SettingsAPI.getCustomProvider(providerId);
      setProvider(data);
    } finally {
      setLoading(false);
    }
  ...
  const handleDelete = useCallback(async () => {
    setDeleting(true);
    try {
      await WorkspaceV2SettingsAPI.deleteCustomProvider(providerId);
      onDeleted();
    } catch (e) {
  ...
  const handleTest = useCallback(async () => {
    ...
    if (!centerLoading) setTestResult(null);
    try {
      const res = await WorkspaceV2SettingsAPI.testCustomConnectionById(provider.id);
      if (centerLoading) {
