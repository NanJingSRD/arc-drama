  ...
  return (
    <div ref={containerRef} className={`relative ${className || ""}`}>
      ...
      <button
        ...
        className={
          compact
            ? `flex w-full items-center justify-between gap-2 rounded-[7px] border border-hairline bg-bg-grad-a/55 px-2.5 py-1.5 text-[12.5px] text-text transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent ${disabled ? "cursor-not-allowed opacity-70" : "hover:border-hairline-strong hover:bg-bg-grad-a/65"}`
            : `flex w-full items-center justify-between gap-2 rounded-[8px] border border-hairline bg-bg-grad-a/55 px-3 py-2 text-[13px] text-text transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent ${disabled ? "cursor-not-allowed opacity-70" : "hover:border-hairline-strong hover:bg-bg-grad-a/65"}`
        }
      >
        <span className={`truncate ${showFallback ? "text-text-3" : ""}`}>{displayText}</span>
        <ChevronDown
          className={`shrink-0 text-text-4 transition-transform ${open ? "rotate-180" : ""} ${compact ? "h-3.5 w-3.5" : "h-4 w-4"}`}
        />
      </button>
