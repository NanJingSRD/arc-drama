...
interface Props {
  type: AssetType;
  mode: Mode;
  /** 创建时锁定资产类型（不可切换）；类型仍由 type 传入并由调用方提交接口 */
  typeLocked?: boolean;
...
export function AssetFormModal({
  type, mode, typeLocked = false, initialData, previewImageUrl, conflictWith, onClose, onSubmit,
}: Props) {
  ...
  /** 场景/道具新增不展示参考图上传；人物及编辑/入库仍保留 */
  const showImageUploader = isCharacter || mode !== "create";
  const typeLabel = t(`type.${type}`);
  const title =
    mode === "create" ? t("create_title", { type: typeLabel })
  ...
  return (
    <GlassModal
      ...
      onClose={onClose}
      labelledBy={titleId}
      widthClassName="w-[480px] max-w-[96vw]"
    >
      {/* Header */}
        <div
        ...
        {/* Body：名称 → 图片 → 描述（资产类型不展示，由 type prop 锁定并由调用方传接口） */}
        <div className="flex flex-col gap-4 p-6">
          ...
          {showImageUploader ? imageUploader : null}
