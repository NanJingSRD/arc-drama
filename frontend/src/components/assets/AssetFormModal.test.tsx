...
describe("AssetFormModal", () => {
  ...
  it("typeLocked create uses typed title and hides asset type field", () => {
    render(
      <AssetFormModal
        type="character"
        mode="create"
    ...
    expect(screen.getByRole("heading", { name: "create_title" })).toBeInTheDocument();
    expect(screen.queryByLabelText(/field\.type/)).not.toBeInTheDocument();
    expect(screen.queryByText("add_asset")).not.toBeInTheDocument();
  });

  it("hides image upload for scene and prop create", () => {
    const { rerender } = render(
      <AssetFormModal
        type="scene"
        mode="create"
    ...
    expect(screen.queryByText("upload_image_hint")).not.toBeInTheDocument();
    ...
    expect(screen.getByText("upload_image_hint")).toBeInTheDocument();
  });
});
