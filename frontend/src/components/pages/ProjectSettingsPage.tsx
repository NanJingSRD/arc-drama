...
function SectionCard({ kicker, title, description, children, footer }: SectionCardProps) {
  return (
    <section className="overflow-hidden rounded-[12px]" style={W3_CARD_STYLE}>
      <header className="border-b px-5 pt-4 pb-3" style={{ borderColor: W3.borderSoft }}>
        <div className="font-mono text-[10px] font-bold uppercase tracking-[0.16em]" style={w3KickerStyle()}>
          {kicker}
        </div>
        {title ? (
      ...
      {footer ? (
        <footer className="px-5 py-3" style={W3_CARD_FOOTER_STYLE}>
          {footer}
        </footer>
      ) : null}
    </section>
...
export function ProjectSettingsPage() {
  ...
  return (
    <WorkspacePageShell fixed>
        ...
        <div className="mx-auto max-w-3xl px-6 py-7 pb-24 space-y-5">
                        ...
                        <span className="inline-flex items-center gap-2">
                          <span
                            ...
                            style={{
                              ...
                              background:
                                aspectRatio === ar ? "rgba(34,211,238,0.18)" : "transparent",
                            }}
                          />
          ...
          {!options && (
            <div className="flex items-center gap-2 py-6 text-text-3">
              <Loader2 className="h-3.5 w-3.5 motion-safe:animate-spin" style={{ color: W3.cyan }} aria-hidden />
      ...
      {/* ─── Sticky save bar ─── */}
      <footer className="shrink-0" style={W3_FOOTER_BAR_STYLE}>
        <div className="mx-auto flex max-w-3xl items-center justify-between gap-3 px-6 py-3">
          ...
          <div className="flex shrink-0 items-center gap-2">
            <button
              onClick={() => guardedNavigate(`/app/projects/${projectName}`)}
              className={`${GHOST_BTN_LG_CLS} rounded-[7px] focus-visible:ring-cyan-400`}
            >
              {t("common:cancel")}
            </button>
            <button
              ...
              disabled={saving}
              className={`${ACCENT_BTN_CLS} rounded-[7px] px-5`}
              style={W3_ACCENT_BUTTON_STYLE}
            >
