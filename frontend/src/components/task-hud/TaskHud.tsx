...
function ChannelSection({
  ...
  tasks: TaskItem[];
  onCancel?: (taskId: string) => void;
  channelHeaderStyle?: React.CSSProperties;
}) {
  const { t } = useTranslation("dashboard");
  ...
  const recent = terminal.filter((task) => !hiddenIds.has(task.task_id)).slice(0, 5);
...
export function TaskHud({
  anchorRef,
  channelsMaxHeightClass = "max-h-80",
  channelHeaderStyle,
}: {
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Channels */}
        <div
          className={`${channelsMaxHeightClass} overflow-y-auto`}
          style={{ borderBottom: "1px solid var(--color-hairline-soft)" }}
        >
          <ChannelSection
            ...
            channelHeaderStyle={channelHeaderStyle}
          />
          <div
            className="h-px"
