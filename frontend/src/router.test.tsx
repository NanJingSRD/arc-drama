...
vi.mock("@/components/auth/WeChatLoginModal", () => ({
  WeChatLoginModal: () => null,
}));
...
describe("AppRoutes", () => {
  ...
  it("redirects root path to /app/home", async () => {
    renderAt("/");
    expect(await screen.findByTestId("welcome-page")).toBeInTheDocument();
  });

  it("redirects /app to /app/home", async () => {
    renderAt("/app");
    expect(await screen.findByTestId("welcome-page")).toBeInTheDocument();
  });
  ...
  it("allows unauthenticated access to welcome home", async () => {
    useAuthStore.setState({ isAuthenticated: false, isLoading: false });
    renderAt("/app/home");
    expect(await screen.findByTestId("welcome-page")).toBeInTheDocument();
  });

  it("shows workspace login gate for unauthenticated nested project URL", async () => {
    useAuthStore.setState({ isAuthenticated: false, isLoading: false });
    renderAt("/app/projects/demo");
    expect(await screen.findByText("登录后即可访问工作空间")).toBeInTheDocument();
    expect(screen.queryByTestId("studio-layout")).not.toBeInTheDocument();
  });

  it("shows workspace login gate for unauthenticated projects list", async () => {
    useAuthStore.setState({ isAuthenticated: false, isLoading: false });
