...
import { API_BASE, getAppBase, isAppPath } from "@/utils/app-base";
import {
  isWorkspaceAppPath,
  WORKSPACE_REQUIRES_WECHAT_AUTH,
...
import { getToken, clearToken, clearAuthUserProfile, normalizeAccessToken } from "@/utils/auth";
...
export { API_BASE } from "@/utils/app-base";
...
class API {
  ...
  /** 构造合成视频下载 URL */
  static getMergedVideoDownloadUrl(
    projectName: string,
    episode: number,
    downloadToken: string,
  ): string {
    return `${API_BASE}/projects/${encodeURIComponent(projectName)}/export/merged-video?episode=${encodeURIComponent(episode)}&download_token=${encodeURIComponent(downloadToken)}`;
  }
  ...
  /** 直接调用 LLM 生成剧集（绕过 Agent SDK）。 */
  static async directGenerateEpisode(
    projectName: string,
    payload: { episode?: number; title?: string | null }
  ): Promise<{
    success: boolean;
    message: string;
    ...
    return this.request(
      `/projects/${encodeURIComponent(projectName)}/episodes/direct-generate`,
      {
        method: "POST",
        body: JSON.stringify(payload),
      }
  ...
  static async autoGenerateAssets(
    projectName: string,
    payload: { asset_type: "character" | "scene" | "prop" | "all" }
  ): Promise<{
    success: boolean;
    message: string;
    task_id?: string;
    ...
    return this.request(
      `/projects/${encodeURIComponent(projectName)}/auto-assets/generate`,
      {
        method: "POST",
        body: JSON.stringify(payload),
      }
   ...
   * 上传单个分镜到外部平台
   */
  static async uploadStoryboard(
    projectName: string,
    segmentId: string,
  ): Promise<{ success: boolean; task_id: string; message: string }> {
    return this.request("/storyboard/upload/", {
      method: "POST",
      body: JSON.stringify({ project_name: projectName, segment_id: segmentId }),
    });
  }
   ...
   * 批量上传分镜到外部平台
   */
  static async uploadStoryboardsBatch(
    projectName: string,
    segmentIds: string[],
  ): Promise<{ success: boolean; task_ids: string[]; message: string }> {
    return this.request("/storyboard/upload/batch", {
      method: "POST",
      body: JSON.stringify({ project_name: projectName, segment_ids: segmentIds }),
    });
  }
