...
describe("API", () => {
  ...
  describe("request", () => {
    ...
    it("clears auth and redirects on unauthorized responses", async () => {
      ...
      // /app（无尾斜杠）不属于 /app/ 受保护页面，重定向到首页拉起微信登录。
      ...
      expect(location.href).toBe("/app/home?wechat=1");
    });

    it("redirects to home with wechat reauth when unauthorized on /app/* pages", async () => {
      const fetchMock = vi.fn().mockResolvedValue(
      ...
      expect(location.href).toBe("/app/home?wechat=1&from=%2Fapp%2Fprojects%2Fdemo");
    });
  });
  ...
  describe("request-based wrappers", () => {
    it("covers project, character, scene, prop, product, script and generation endpoints", async () => {
      ...
      await API.uploadStoryboard("demo", "E1S01");
      await API.uploadStoryboardsBatch("demo", ["E1S01", "E1S02"]);
      ...
      expect(requestSpy).toHaveBeenCalledWith("/storyboard/upload/", {
        method: "POST",
        body: JSON.stringify({ project_name: "demo", segment_id: "E1S01" }),
      });
      expect(requestSpy).toHaveBeenCalledWith("/storyboard/upload/batch", {
        method: "POST",
        body: JSON.stringify({ project_name: "demo", segment_ids: ["E1S01", "E1S02"] }),
      });
    });
  ...
  describe("fetch-based wrappers", () => {
    ...
    describe("downloadDiagnostics", () => {
      ...
      it("triggers unauthorized handling on 401", async () => {
        ...
        expect(clearTokenMock).toHaveBeenCalledTimes(1);
        expect(location.href).toBe("/app/home?wechat=1&from=%2Fapp%2Fsettings");
      });
